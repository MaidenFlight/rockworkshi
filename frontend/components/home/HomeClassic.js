// THE CLASSIC HOMEPAGE — the composition as it shipped at 5389266.
//
// Kept rather than deleted so the world toggle can restore the whole old site,
// not merely its colours: this is the one page whose STRUCTURE changed in the
// redesign, so a token swap alone would have produced a hybrid that never
// existed. Everything below is the previous world's work verbatim, including
// its comments, which are the record of why each section is shaped as it is.
//
// It is rendered alongside the Silkie composition and hidden with display:none
// by [data-world], which keeps the switch instant and paint-free. display:none
// also removes it from the accessibility tree, so only one homepage is ever
